...
Airspace and border incursions: once a drone, aircraft or missile has entered the territory or airspace of LT, LV, EE or PL, the incursion is "negative" — even if it was then shot down, forced down, found crashed, or is being investigated. The violation already happened; the shoot-down is the response to it, not a thwarted attack. Air alerts, airport closures and shelter warnings caused by a real incursion are part of the same "negative" incident. An object the military or authorities say they tracked crossing that territory is an incursion even if it has not been identified. "positive" is only for something stopped before it crossed, or for the response reported on its own without the incursion (new air-defence deployments, arrests of the operators). A contact that turned out to be a false alarm — birds, a balloon, a radar artefact — is "neutral".
